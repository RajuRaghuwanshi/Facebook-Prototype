...
public class UserService {
	...
	public Profile login(String emailID, String password) {
		...
		try {
			preparedstmnt = connection
					.prepareStatement("select user_id from login where userName = ? and password = ?");
		...
		try {
			if (nameOfString.length > 1) {
				pst = connection.prepareStatement(
						"select * from profile where (firstName LIKE ? and lastName LIKE ?) and user_id_fk<>?");
				pst.setString(1, "%" + nameOfString[0] + "%");
				pst.setString(2, "%" + nameOfString[1] + "%");
				pst.setInt(3, Integer.parseInt(userId));
			} else {
				pst = connection.prepareStatement(
						"select * from profile where (firstName LIKE ? or lastName LIKE ?) and user_id_fk<>?");
	...
	public Profile setUserInfo(String emailID, String password, String firstName, String lastName, String date_of_birth,
		...
		if (validateEmailId(emailID) == 0) {
			...
			try {
				...
				preparedstmnt = connection.prepareStatement("select user_id from login where userName=?");
				preparedstmnt.setString(1, emailID);
				...
				preparedstmnt = connection.prepareStatement(
						"INSERT INTO `profile`(`user_id_fk`, `firstName`, `lastName`, `dob`, `gender`, `contact`, `homeTown`) VALUES (?,?,?,?,?,?,?)");
				...
				profile = new Profile(uid, firstName, lastName, Date.valueOf(date_of_birth), gender.charAt(0),
						contact_info, hometown, "Excellence", "Bangalore", "JEC", "CISCO", "RDVV");
				if (profile != null) {
					System.out.println(profile.getUser_id());
					new File(PathSetup.imagePath + "users/" + profile.getUser_id() + "/images").mkdirs();
					InputStream fileInputStream = null;
					OutputStream outputStream = null;
					String path = PathSetup.imagePath + "users/" + profile.getUser_id() + "/";

					try {
						fileInputStream = new FileInputStream(PathSetup.imagePath + "defaultProfilePic.jpg");

						outputStream = new FileOutputStream(new File(path + "profilePic.jpg"));
						int read = 0;
	...
	public String getUserFullName(int user_id) {
		...
		try {
			...
			if (rs.next()) {
				...
				System.out.println("getfullName fullName:" + firstName + " " + lastName);
	...
	public List<Profile> getFilterFriendList(int user_id, String filter) {
		...
		Profile p = getUserProfile(user_id);
		String search = null;

		if (filter.equalsIgnoreCase("homeTown")) {
			search = p.getHome_town();

		} else if (filter.equalsIgnoreCase("currentCity")) {

			search = p.getCurrent_city();
		} else if (filter.equalsIgnoreCase("college")) {
			search = p.getCollege();

		} else if (filter.equalsIgnoreCase("employer")) {

			search = p.getEmployer();
		} else if (filter.equalsIgnoreCase("graduateSchool")) {

			search = p.getGraduate_school();
		} else if (filter.equalsIgnoreCase("contact")) {

			search = p.getContact_no();
		} else if (filter.equalsIgnoreCase("dob")) {

			search = sdf.format(p.getDate_of_birth());
		} else {
			search = p.getHigh_school();
		}
		search = search.trim();
		...
		try {
			...
			System.out.println(connection);
			String sql = "select f.friend_id_fk from friendlist f INNER JOIN profile p where (f.user_id_fk= ?) AND (f.friend_id_fk = p.user_id_fk) AND p."
					+ filter + "=?";
