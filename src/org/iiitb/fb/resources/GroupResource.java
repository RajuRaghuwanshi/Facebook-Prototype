...
public class GroupResource {
	...
	public Response groupCreation(@FormParam("group_name") String groupname, @PathParam("user_id") int user_id) {
	...
	public Response updateGroupMemberStatus(@PathParam("user_id") int user_id, Group group,
			@QueryParam("status") String st) {
		...
		if (status == 0) {
			...
			Profile profile = groupservice.addMemberByAdmin(group.getGroup_id(), user_id);
			return Response.ok().entity(profile).build();
		} else if (status == 1) {
			result = groupservice.updateGroupMemberStatus(group.getGroup_id(), user_id, status);

		} else {
			result = groupservice.DeleteGroupMemberStatus(group.getGroup_id(), user_id);
	...
	public Response getGroupList(@PathParam("user_id") int user_id) {
		...
		if (listOfMyGroup.isEmpty()) {
			...
			return Response.ok().entity(new GenericEntity<List<Group>>(listOfMyGroup) {
			}).build();
		}
	}
	...
	public Response getGroupMembersList(@PathParam("group_id") int groupid) {
		...
		List<Profile> listofgroupmembers = groupservice.getMembersOfAGroup(groupid);
		if (listofgroupmembers.isEmpty()) {
			return Response.noContent().build();
		} else {

			return Response.ok().entity(new GenericEntity<List<Profile>>(listofgroupmembers) {
			}).build();
	...
	public Response getGroupJoinRequestList(@PathParam("user_id") int userid, @PathParam("group_id") int groupid) {
		...
		listofgroupjoins = groupservice.getGroupJoinRequests(userid, groupid);
		if (listofgroupjoins == null) {
			return Response.noContent().build();
		} else {

			return Response.ok().entity(new GenericEntity<List<Profile>>(listofgroupjoins) {
			}).build();
	...
	public Response AddMember(@PathParam("user_id") int userid, @PathParam("group_id") int groupid, Profile profile) {
		GroupService groupservice = new GroupService();
	...
	@Produces(value = { MediaType.APPLICATION_JSON })
	@Consumes(value = { MediaType.APPLICATION_JSON })
	public Response joinGroup(@PathParam("member_id") Integer member_id, @PathParam("group_id") Integer group_id) {
	...
	@Produces(value = { MediaType.APPLICATION_JSON })
	@Consumes(value = { MediaType.APPLICATION_JSON })
	public Response leaveGroup(@PathParam("member_id") Integer member_id, @PathParam("group_id") Integer group_id) {
		int result = 0;
		GroupService groupService = new GroupService();
	...
	@Produces(value = { MediaType.APPLICATION_JSON })
	public Response createGroupPost(@PathParam("group_id") String group_id, @PathParam("user_id") String user_id,
		...
		System.out.println("group post input :" + group_id + "  " + fileInputStream + "  " + postdata + "  "
		...
		Post post = new GroupService().createPostInGroup(fileInputStream, fileName, postdata, Integer.parseInt(user_id),
	...
	public Response getAllGroupPost(@PathParam("group_id") String group_id) {
		...
		List<Post> listOfPost = new GroupService().getAllGroupPost(Integer.parseInt(group_id));

		if (listOfPost.isEmpty()) {
			return Response.noContent().build();
		} else {
			return Response.ok().entity(new GenericEntity<List<Post>>(listOfPost) {
			}).build();
		}
	...
	@POST
	@Path("users/{user_id}/groupfilter")
	@Produces(value = { MediaType.APPLICATION_JSON })
	@Consumes(value = { MediaType.APPLICATION_FORM_URLENCODED })
	public Response groupFilter(@FormParam("emailid") String emailid, @PathParam("user_id") int user_id) {

		System.out.println("eventFilter"+emailid+"  user id:"+user_id);

		List<Group> groupList = new GroupService().filterGroup(user_id, emailid);

		if (groupList.isEmpty())
			return Response.noContent().build();
		else
			return Response.ok().entity(new GenericEntity<List<Group>>(groupList) {
			}).build();
	}

	@GET
	@Path("groupfilter/{groupid}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response groupFilterProfile(@PathParam("groupid") int groupid,@QueryParam("userid")int userid,@QueryParam("emailid")String emailid){
		
		System.out.println(groupid);
		
		List<Profile> list = new GroupService().getGroupFilterProfileList(groupid,userid,emailid);
		
		System.out.println("eventFilterProfile"+list.size());
