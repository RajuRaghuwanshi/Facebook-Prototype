...
public class EventResource {
	...
	public Response eventCreation(@FormParam("event_name") String eventname, @FormParam("location") String loc,
			@FormParam("start_date") Date start, @FormParam("end_date") Date end, @FormParam("description") String desc,
		...
		Event event = eventservice.createEvent(eventname, user_id, start, end, loc, desc);
	...
	public Response getEventList(@PathParam("user_id") int user_id) {
		...
		if (eventList.isEmpty())
			...
			return Response.ok().entity(new GenericEntity<List<Event>>(eventList) {
			}).build();
	}
	...
	public Response getAllGroupPost(@PathParam("event_id") String event_id) {
		List<Post> listOfPost = new EventService().getAllEventPost(Integer.parseInt(event_id));
	...
	@Produces(value = { MediaType.APPLICATION_JSON })
	public Response createEventPost(@PathParam("event_id") String event_id, @PathParam("user_id") String user_id,
		...
		Post post = new EventService().createPostInEvent(fileInputStream, fileName, postdata, Integer.parseInt(user_id),
	...
	public Response changeInviteStatus(@PathParam("user_id") Integer user_id, @PathParam("event_id") Integer event_id,
	...
	@Produces(value = { MediaType.APPLICATION_JSON })
	@Consumes(value = { MediaType.APPLICATION_JSON })
	public Response changeDiscoveryStatus(@PathParam("user_id") Integer user_id,
			@PathParam("event_id") Integer event_id, @PathParam("status") String status) {
		int res;
		if (status.equals("ignore"))
			res = new EventService().deleteEvent(event_id, user_id);
		else
			res = new EventService().changeDiscoveryStatus(event_id, user_id, status);
	...
	public Response countGoingAndInterested(@PathParam("event_id") int event_id) {
		...
		// if(count.length>0)
		s = "" + count[0] + "_" + count[1];
		// System.out.println(count1.get(0)+"
		// "+count1.get(1)+"...............");
		if (s.isEmpty())
			return Response.ok().entity(s).build();
		else {
			System.out.println("getting count in resource : " + count[0] + " , " + count[1]);
			return Response.ok().entity(s).build();
		}
	}
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response getFriendsToInvite(@PathParam("user_id") String user_id, @PathParam("event_id") String event_id) {
		EventService eventService = new EventService();
		ArrayList<Profile> inviteList = eventService.getFriendsToInvite(user_id, event_id);
		if (inviteList.isEmpty())
			return Response.noContent().build();
		return Response.ok().entity(new GenericEntity<List<Profile>>(inviteList) {
		}).build();
	}
	...
	// @Consumes
	public Response sendFriendInvitation(@PathParam("user_id") String user_id, @PathParam("event_id") String event_id) {
		EventService eventService = new EventService();
	...
	public Response getDiscoverEvent(@PathParam("id") int user_id) {
		List<Event> discover_list = new EventService().getDiscoverEvent(user_id);
		if (discover_list == null)
			return Response.noContent().build();
		else {
			System.out.println("in resource : " + discover_list);
			return Response.ok().entity(new GenericEntity<List<Event>>(discover_list) {
			}).build();
		}
	}
	...
	@POST
	@Path("users/{user_id}/eventfilter")
	@Produces(value = { MediaType.APPLICATION_JSON })
	@Consumes(value = { MediaType.APPLICATION_FORM_URLENCODED })
	public Response eventFilter(@FormParam("emailid") String emailid, @PathParam("user_id") int user_id) {

		System.out.println("eventFilter"+emailid+"  user id:"+user_id);

		List<Event> eventList = new EventService().filterEvents(user_id, emailid);
	...
	@GET
	@Path("eventsFilter/{eventid}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response eventFilterProfile(@PathParam("eventid") int eventid){
		
		System.out.println(eventid);
		
		List<Profile> list = new EventService().getEventProfiles(eventid);
